...
public class GamePanel extends JPanel implements ActionListener {
		...
		if (running) {
			...
			for (int i = 0; i < bodyParts; i++) {
				if (i == 0) {
					...
					g.setColor(new Color(R + 50, G + 50, B + 50));
				...
				g.setColor(new Color(0xA5F1E9));
				g.setFont(new Font("Pixeloid Sans", Font.PLAIN, 40));
	...
	public void checkCollisions() {
		...
		if (x[0] < 0)
			running = false;// touch left
		if (x[0] > SCREEN_WIDTH)
			running = false;// touch right
		if (y[0] < 0)
			running = false;// touch top
		if (y[0] > SCREEN_HEIGHT)
			running = false;// touch bottom
		if (!running) {
			timer.stop();
		}
	}
	...
	public void gameOver(Graphics g) {
		...
		g.setColor(Color.red);
		g.setFont(new Font("Pixeloid Sans", Font.PLAIN, 75));
